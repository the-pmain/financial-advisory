...
describe('isUuid / isDocumentKind', () => {
  it('accepts a uuid and known kinds', () => {
    ...
    assert.equal(isDocumentKind('brochure'), true);
...
describe('normalizeDocuments / persistDocuments', () => {
  it('always returns the flattened seven-kind view', () => {
    const view = normalizeDocuments(null);
    assert.deepEqual(view, emptyDocuments());
  });

  it('flattens nested claim kinds and persists the 4-key shape', () => {
    ...
    assert.deepEqual(Object.keys(persisted).sort(), ['agreement', 'brochure', 'claim', 'release']);
    assert.equal(persisted.brochure, null);
